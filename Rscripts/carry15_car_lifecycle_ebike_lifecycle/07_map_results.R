...
#---------- Figure 4 net  ---------------------   
  ...
  #work out the bins  
  summary(lsoa$meanMDDco2save)  
  summary(lsoa$sumAllEbikeco2save)
  summary(lsoa$meandiffMDDco2save_AllEbikeco2save)
  summary(lsoa$meandiffAllbike_AllEbikeco2save)
  #fill fixed
  vecForBins <- lsoa$meandiffAllbike_AllEbikeco2save
  vecForBins <- as.vector(append(vecForBins,lsoa$meandiffMDDco2save_AllEbikeco2save)) 
  
  summary(vecForBins)
  hist(vecForBins,breaks = 100)
  
  #manial breaks appropriate cor car lifecycle e-bike lifecycle
  breakVec <- c(0,0.15,0.26,0.31,0.41,2.4)
  ...
  m <- lsoa2 %>%
    tm_shape() +
    tm_fill(namei,lty = 0, lwd = 0,border.alpha = 0.2,breaks = breakVec,
  ...
  tmap_save(m,paste0("plotsmaps",datetime,"/",namei,"FIG4manbreaksRdYlBu.jpg"),
            height = 29.7,width = 21, unit = "cm",
            dpi = 300)
...
########################  #--------- alternative man breaks ---------------------------------------------
  
  
  #-----key comparable maps (maps with the same class boundaries and colour scheme)--
  
  #breaks for car lifecycle e-bike lifecycle
  breakVec <- c(0,0.05,0.1,0.2,0.4,0.8,2.4)
  
  #------- mdd e-bike -------------
  # we didn't put this comparison in the paper for reasons of space, 
  ...
  tmap_save(m,paste0("plotsmaps",datetime,"/",namei,"FIG5ALTmanbreaksRdYlBu.jpg"),
            height = 29.7,width = 21, unit = "cm",
            dpi = 300)
  
  #----------- meandiffAllbike_AllEbikeco2save
  breakVec <- c(0,0.05,0.1,0.2,0.4,0.8,1.44)
  ...
  tmap_save(m,paste0("plotsmaps",datetime,"/",namei,"FIG4ALTmanbreaksRdYlBu.jpg"),
            height = 29.7,width = 21, unit = "cm",
            dpi = 300)
